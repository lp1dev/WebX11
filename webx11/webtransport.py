...
class WebTransportHandler:
    ...
    def _send_frame(self, frame):
        """Put one serialized tile frame on a fresh unidirectional stream."""
        ...
        self.protocol._quic.send_stream_data(
            stream_id=stream_id, data=frame, end_stream=True)
        self.protocol.transmit()

    async def send_keyframe(self):
        """Seed this session with a full keyframe without disturbing the diff
        baseline shared across clients of the display."""
        try:
            window_display = self.window_manager.get_display(self.display_id)
            ...
            print(f"Error sending keyframe: {e}")
    ...
    async def send_window_update(self, force=False):
        """Capture a diff (or keyframe when forced) off the event loop and
        stream it. The serialized tile frame carries its own header, so it goes
        straight onto the stream with no extra envelope."""
        global IMAGES_SENT, LAST_FRAME
        try:
            # Frame pacing; never throttle a forced keyframe.
            if not force:
            ...
            loop = asyncio.get_running_loop()
            window_image = await loop.run_in_executor(
                window_display.capture_executor,
                window_display.capture_window, False, force)

            if window_image:
                IMAGES_SENT += 1
                self._send_frame(window_image)
                await asyncio.sleep(0)
...
class WebTransportProtocol(QuicConnectionProtocol):
    ...
    def _handshake_webtransport(self, stream_id: int, request_headers: dict):
        ...
        # Seed the client with a full keyframe (non-disturbing); diffs follow
        # from the update loop.
        asyncio.create_task(self._handler.send_keyframe())
        ...
        try:
            ...
            if not os.path.exists("certs/cert.pem") or not os.path.exists("certs/key.pem"):
                ...
                subprocess.run([
                    ...
                    "-subj", f"/CN={host}", "-addext", "subjectAltName = DNS:localhost"
